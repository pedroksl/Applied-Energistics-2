...
public final class GridLayoutManager extends AbstractLayout {
    ...
    public LytSize maximumLayoutSize(final Container target) {
        return new LytSize(Integer.MAX_VALUE, Integer.MAX_VALUE);
    }

    public LytSize minimumLayoutSize(final Container container) {
        ...
        var result = MutableSize.of(getTotalGap(container, horizontalInfo, verticalInfo));
    ...
    public LytSize preferredLayoutSize(final Container container) {
        validateInfos(container);
    ...
    private LytSize getTotalGap(final Container container, final DimensionInfo hInfo, final DimensionInfo vInfo) {
    ...
    private static boolean shouldAddGapAfterCell(final DimensionInfo info, final int cellIndex) {
        ...
        for (int i = 0; i < info.getComponentCount(); i++) {
            final LayoutElement component = info.getComponent(i);
    ...
    public void layoutContainer(final Container container) {
        ...
        final LytSize gap = getTotalGap(container, horizontalInfo, verticalInfo);

        var size = MutableSize.of(container.getSize());
        size.width -= gap.width();
        size.height -= gap.height();

        var prefSize = MutableSize.of(preferredLayoutSize(container));
        prefSize.width -= gap.width();
        prefSize.height -= gap.height();

        var minSize = MutableSize.of(minimumLayoutSize(container));
        ...
        for (int i = 0; i < layoutState.getComponentCount(); i++) {
            ...
            final LayoutElement component = layoutState.getComponent(i);
            ...
            var componentSize = new MutableSize(cellWidth, cellHeight);
    ...
    private static void updateSizesFromChildren(final DimensionInfo info, final boolean min, final int[] widths) {
        for(int i=info.getComponentCount() - 1; i >= 0; i--) {
            LayoutElement child = info.getComponent(i);
            ...
            if (c.isUseParentLayout() && child instanceof Container) {
                ...
                else if (container.getChildren().size() == 1 && container.getChildren().get(0) instanceof Container childContainer) {
                    // "use parent layout" also needs to work in cases where a grid is the only control in a non-grid panel
                    // which is contained in a grid
