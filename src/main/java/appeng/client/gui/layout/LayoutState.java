...
public final class LayoutState {
    ...
    public LayoutState(final GridLayoutManager layout, final boolean ignoreInvisibleComponents) {
        ...
        for (int i=0; i < layout.getComponentCount(); i++){
            final LayoutElement component = layout.getComponent(i);
        ...
        myComponents = (LayoutElement[])componentsList.toArray(new LayoutElement[0]);
        myConstraints = (GridConstraints[])constraintsList.toArray(GridConstraints.EMPTY_ARRAY);

        myMinimumSizes = new LytSize[myComponents.length];
        myPreferredSizes = new LytSize[myComponents.length];
    ...
    public LayoutElement getComponent(final int index){
